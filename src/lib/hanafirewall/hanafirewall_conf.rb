...
module HANAFirewall
    ...
    def get_service_names
          Dir.glob('/etc/hana-firewall/*').map{ |path|
            file_name = Pathname.new(path).basename.to_s

            # The firewall program calculates short name by keeping numbers and letters, write '-' for other characters, and convert the whole string into lower case.
            short_name = ''
            file_name.each_codepoint{ |c|
              if /\p{L}/.match(c.chr)
                short_name += c.chr
    ...
    def get_inst_numbers
      HANAFirewall::SysconfigEditor.new(IO.read('/etc/sysconfig/hana-firewall')).get('HANA_INSTANCE_NUMBERS').split(/\s+/)
    end

    # set_inst_numbers writes down a new string array value into HANA_INSTANCE_NUMBERS configuration key.
    ...
    def get_zone_services
      ...
      Hash[
          ::Y2Firewall::Firewalld.instance.zones.map { |zone|
    ...
    def regen_svcs
      ...
      if status.exitstatus != 0
        raise "Command line error - " + outerr.gets
      end
    end
end
