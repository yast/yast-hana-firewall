...
module HANAFirewall
  ...
  class MainDialog
    ...
    def run
                ...
                Left(
                  Frame(
                    ...
                    VBox(
                      ...
                      Left(InputField(Id(:inst_numbers), _("Instance numbers"),
                        get_inst_numbers.join(" ")))
                    )
                  )
                ),
      ...
      if !package_present && Popup.YesNo(_("Install HANA-Firewall package?")) &&
          Package.DoInstall(["HANA-Firewall"])
        package_present = true
      end
      return :finish_dialog if !package_present
      ...
      UI.ChangeWidget(Id(:selected_svcs), :Value, @zone_services.keys.min)
    ...
    def event_loop
      loop do
        ...
        when :ok
          ...
          restart_service("firewalld") if UI.QueryWidget(Id(:reload), :Value)
          return
        when :zones
          load_for_selected_zone
        when :add_svc
          ...
          if svc && zone_name
            ...
            ::Y2Firewall::Firewalld.instance.zones.find { |z| z.name == zone_name }
              .services += [svc]
            load_for_selected_zone
          end
        when :del_svc
        ...
        when :help
          Popup.LongMessageGeometry(
            _(
              "HANA firewall is not an independent firewall! " \
              "It is a utility for firewalld.\n" \
              "The command line tool generates firewalld service definitions, and this " \
              "graphical tool assigns those services to zones.\n" \
              "You must use firewalld controls (such as firewall-cmd command line) " \
              "to manipulate the actual firewall setup, such as interface assignment."
            ),
            60, 16
          )
        when :cancel
  ...
  private
    ...
    def restart_service(name)
      service_api = if defined?(Yast2::Systemd::Service)
